...
contract TestCarrotRouter is ICarrotSwapCallback {
    ...
    function carrotSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? ICarrotPool(msg.sender).token0()
                : ICarrotPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == ICarrotPool(pools[0]).token1();
            ICarrotPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
